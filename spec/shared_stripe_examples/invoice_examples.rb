...
shared_examples 'Invoice API' do
  ...
  context "creating a new invoice" do
    it "creates a stripe invoice" do
      ...
      expect(invoice.billing_reason).to eq('manual')
      expect(invoice.collection_method).to eq('charge_automatically')
      expect(invoice.payment_intent).to be_nil
      expect(invoice.amount_remaining).to eq(invoice.amount_due)
    end
  ...
  context "sending an invoice" do
    ...
    it 'expands payment_intent.payment_method when requested' do
      sent_invoice = Stripe::Invoice.send_invoice(
        @invoice.id, expand: ['payment_intent.payment_method']
      )
      expect(sent_invoice.payment_intent).to respond_to(:status)
      expect(sent_invoice.payment_intent.payment_method).to respond_to(:type)
    end
