    ...
    module Helpers
      ...
      def resolve_subscription_changes(subscription, prices, customer, options = {})
        ...
        subscription[:items][:data] = prices.map do |price|
          matching_item = items && items.detect { |item| [item[:price], item[:price]].include? price[:id] }
          if matching_item
            matching_item[:quantity] ||= 1
            matching_item[:id] ||= new_id('si')
            params = matching_item.merge(price: price)
            params[:price] = price if price[:object] == "price"
      ...
      def custom_subscription_params(prices, cus, options = {})
        verify_trial_end(options[:trial_end]) if options[:trial_end]

        price = prices.first if prices.size == 1

        now = Time.now.utc.to_i
        created_time = options[:created] || now
        start_time = options[:current_period_start] || now
        params = { customer: cus[:id], current_period_start: start_time, created: created_time }
        params.merge!({ :price => (prices.size == 1 ? prices.first : nil) })
        ...
        if (((price && price[:trial_period_days]) || 0) == 0 && options[:trial_end].nil?) || options[:trial_end] == "now"
          end_time = options[:billing_cycle_anchor] || get_ending_time(start_time, price)
          ...
          end_time = options[:trial_end] || (Time.now.utc.to_i + price[:trial_period_days]*86400)
      ...
      def add_subscription_to_customer(cus, sub, pi=nil)
        ...
        if cus[:currency].nil?
          cus[:currency] = sub[:items][:data][0][:price][:currency]
        elsif cus[:currency] != sub[:items][:data][0][:price][:currency]
      ...
      # `intervals` is set to 1 when calculating current_period_end from current_period_start & price
      # `intervals` is set to 2 when calculating Stripe::Invoice.upcoming end from current_period_start & price
      def get_ending_time(start_time, price, intervals = 1)
        return start_time unless price

        interval = price[:interval] || price.dig(:recurring, :interval)
        interval_count = price[:interval_count] || price.dig(:recurring, :interval_count) || 1
        case interval
        when "week"
