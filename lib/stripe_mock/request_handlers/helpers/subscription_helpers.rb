    ...
    module Helpers
      ...
      def custom_subscription_params(plans, cus, options = {})
        ...
        if (((plan && plan[:trial_period_days]) || 0) == 0 && options[:trial_end].nil?) || options[:trial_end] == "now"
          ...
          end_time = anchor ? get_ending_time(anchor, plan) : get_ending_time(start_time, plan)
          params.merge!({status: 'active', current_period_end: end_time, trial_start: nil, trial_end: nil, billing_cycle_anchor: anchor || created_time})
        else
      ...
      def add_subscription_to_customer(cus, sub)
        if sub[:trial_end].nil? || sub[:trial_end] == "now"
          id = new_id('ch')
          amount = if sub[:plan]
            sub[:plan][:amount] || sub[:plan][:unit_amount]
          else
        ...
        item_data = sub[:items][:data][0]
        item_currency = (item_data[:plan] || item_data[:price])[:currency]
        if cus[:currency].nil?
          cus[:currency] = item_currency
        elsif cus[:currency] != item_currency
